...
/*   Updated: 2024/01/16 16:38:09 by mho              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bresenhem_low(t_data *data, t_2d *p1, t_2d *p2)
{
	...
	if ((p2->y - p1->y) < 0)
		yi = -1;
	d = (2 * ABS(p2->y - p1->y)) - (p2->x - p1->x);
	x = p1->x;
	y = p1->y;
	while (x <= p2->x)
	{
		plot(x, y, get_color(p1->color, p2->color, (p2->x - p1->x), (p1->x
					- x)), data);
		if (d > 0)
		{
			y = y + yi;
...
void	bresenhem_high(t_data *data, t_2d *p1, t_2d *p2)
{
	...
	if ((p2->x - p1->x) < 0)
		xi = -1;
	d = (2 * ABS(p2->x - p1->x)) - (p2->y - p1->y);
	y = p1->y;
	x = p1->x;
	while (y <= p2->y)
	{
		plot(x, y, get_color(p1->color, p2->color, (p2->y - p1->y), (y
					- p1->y)), data);
		if (d > 0)
		{
			x = x + xi;
